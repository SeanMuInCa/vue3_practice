        ...
        <section class="todoapp">
            <header class="header">
                ...
                <input class="new-todo" autofocus="false" autocomplete="off" placeholder="What needs to be done?"
                    v-model="todo.newTodoTitle.value" @keyup.enter="todo.addTodo" />
            </header>
            <section class="main" v-show="list.todos.value.length">
                <input id="toggle-all" class="toggle-all" type="checkbox" @change="editTodo.checkAll($event.target.checked)"
                    :checked="editTodo.checkStatus()" />
                <label for="toggle-all">Mark all as complete</label>
                <ul class="todo-list">
                    <li class="todo" :class="{ completed: todo.isDone, editing: editTodo.editingTodoRef.value === todo }"
                        v-for="todo in filter.filtedData.value" :key="todo.id">
                        <div class="view">
                        ...
                        <input class="edit" type="text" v-model="todo.title" @blur="editTodo.modified(todo)"
                            @keypress.enter="editTodo.modified(todo)" @keyup.escape="editTodo.cancelEdit(todo)" />
                    </li>
                </ul>
            </section>
